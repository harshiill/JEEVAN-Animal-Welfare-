...
export default function ChatBot({ isLoggedIn }: ChatBotProps) {
  ...
  // 🔒 If user is not logged in, hide the entire chatbot
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {open ? (
        <div className="w-[400px] h-[550px] bg-white shadow-2xl rounded-2xl flex flex-col border border-gray-300">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 text-sm bg-blue-50 scrollbar-thin">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`max-w-[90%] px-3 py-2 rounded-lg shadow ${
                  msg.role === 'user'
                    ? 'bg-[#00a89d] text-white self-end ml-auto'
                    : 'bg-white border border-gray-300 text-gray-800 self-start mr-auto'
                }`}
              >
                {msg.content}
              </div>
            ...
            {loading && (
              <div className="bg-white border border-gray-300 px-3 py-2 rounded-lg shadow text-gray-500 italic w-fit">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-blue-300 hover:bg-[#00a89d] text-white px-4 py-2 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105"
        >
